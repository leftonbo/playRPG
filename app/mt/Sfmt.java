...
public class Sfmt
{
    ...
    protected int[]x=new int[624]; /* 状態テーブル */

    protected int[]GetParity()
    { return new int[]{ 0x00000001,0x00000000,0x00000000,0x13c9e684 }; }
    ...
    void period_certification() {
        ...
        index=624; range=0; normal_sw=0; coin_bits=0; byte_pos=0;
    ...
    /*synchronized*/ public void InitMtEx(int[]init_key) {
        ...
        for (i=0;i<624;i++) x[i]=0x8b8b8b8b;
        if (key_len+1>624) c=key_len+1; else c=624;
        r=x[0]^x[306]^x[623]; r=(r^(r>>>27))*1664525;
        x[306]+=r; r+=key_len; x[317]+=r; x[0]=r; c--;
        for (i=1,j=0;j<c&&j<key_len;j++) {
            r=x[i]^x[(i+306)%624]^x[(i+623)%624];
            r=(r^(r>>>27))*1664525; x[(i+306)%624]+=r;
            r+=init_key[j]+i; x[(i+317)%624]+=r;
            x[i]=r; i=(i+1)%624;
        }
        for (;j<c;j++) {
            r=x[i]^x[(i+306)%624]^x[(i+623)%624];
            r=(r^(r>>>27))*1664525; x[(i+306)%624]+=r; r+=i;
            x[(i+317)%624]+=r; x[i]=r; i=(i+1)%624;
        }
        for (j=0;j<624;j++) {
            r=x[i]+x[(i+306)%624]+x[(i+623)%624];
            r=(r^(r>>>27))*1566083941; x[(i+306)%624]^=r;
            r-=i; x[(i+317)%624]^=r; x[i]=r; i=(i+1)%624;
        }
        period_certification();
    }
    ...
    public Sfmt() {
    	int[] init_key = 
    		{(int) System.currentTimeMillis(), (int) Runtime.getRuntime().freeMemory()};
    	InitMtEx(init_key);
	}
